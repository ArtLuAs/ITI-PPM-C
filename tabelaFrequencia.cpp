...
// -------------------------------------------------------------------------
// Implementação do Princípio de Exclusão
// -------------------------------------------------------------------------

void SimpleFrequencyTable::excludeSymbol(uint32_t symbol) {
    uint32_t currentFreq = get(symbol);
    if (currentFreq > 0) {
        // Salva a frequência original antes de zerar
        savedFrequencies.push_back({symbol, currentFreq});
        // Utiliza o set da própria classe, que já atualiza a Fenwick Tree e o total
        set(symbol, 0); 
    }
}

void SimpleFrequencyTable::restoreExcludedSymbols() {
    for (const auto& pair : savedFrequencies) {
        // Restaura as frequências originais na Fenwick Tree
