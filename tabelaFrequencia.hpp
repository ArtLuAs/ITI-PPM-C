 ...
 * after construction. This implementation uses a Fenwick tree (Binary Indexed Tree)
 * to perform updates and cumulative frequency queries in logarithmic time.
 */
class SimpleFrequencyTable final : public FrequencyTable {
	...
	// Fenwick tree to maintain cumulative frequencies efficiently.
	private: std::vector<std::uint32_t> tree;
	...
	// Calculates the prefix sum up to the given index using the Fenwick tree.
	private: std::uint32_t read(std::uint32_t index) const;
