...
CSV_PATH = Path("saidas/benchmark_resultados.csv")
OUT_DIR = Path("saidas")
...
def preparar_dataframe(csv_path: Path) -> pd.DataFrame:
    ...
    # Limpa BOM, espacos e sujeira no cabecalho
    df.columns = (
    ...
    # Compatibilidade com formatos antigos / alternativos
    rename_map = {
    ...
    # Se nao houver Tool, tenta inferir
    ...
    # Se nao houver K, preenche vazio
    ...
    # Se BPS nao existir, tenta calcular
    ...
    # Se Original_bytes nao existir mas houver BPS e Compressed_bytes, tenta reconstruir
...
def plot_barras_bps(df: pd.DataFrame, k_alvo: str):
    ...
    # Mantem apenas PPM-C no K pedido e todos os 7z-PPMd
...
def plot_barras_tempo(df: pd.DataFrame, k_alvo: str, coluna_tempo: str, titulo: str, nome_saida: str):
    ...
    # --- ALTERAÇÃO AQUI: ESCALA LOGARÍTMICA (SEMILOG) ---
    ax.set_yscale("log")
    ...
    # Grid com 'both' para exibir as linhas intermediárias da escala Log
